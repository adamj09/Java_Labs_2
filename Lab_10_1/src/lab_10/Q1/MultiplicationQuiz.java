...
public class MultiplicationQuiz implements Calculable {
    private int num1, num2, min, max;
    private ArrayList<Integer> previousAnswers = new ArrayList<>();

    public MultiplicationQuiz(int min, int max) {
        setRange(min, max);
    }
    ...
    public void run() {
        ...
        System.out.println("Welcome to a basic multiplication quiz! Enter integers as your answers. Type \"exit\" to quit.");
        while(true) {
            setValues();
            while(true) {
